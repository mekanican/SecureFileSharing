...
#              Example: data = open("abc", "rb").read(); BytesIO(data) -> file_data
#   file_name: real name of file
#   content_type: string, MIME type infer from http request!
#                 Example: "image/jpeg", https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types
#   RESULT: Json of bucket_name (const), filename modified with timestamp and url to bucket
# 
# check_file_name_exists: Check filename is in Minio, make sure checking before download
#   bucket_name: default to const MinioClient.bucket_name (replace class with object)
